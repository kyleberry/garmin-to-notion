...
def activity_needs_update(existing_activity, new_activity):
    ...
    return (
        existing_props['Distance (mi)']['number'] != round(new_activity.get('distance', 0) / 1000 * 0.621371, 2) or
...
def create_activity(client, database_id, activity):
    ...
    properties = {
        ...
        "Distance (mi)": {"number": round(activity.get('distance', 0) / 1000 * 0.621371, 2)},
...
def update_activity(client, existing_activity, new_activity):
    ...
    properties = {
        "Activity Type": {"select": {"name": activity_type}},
        "Subactivity Type": {"select": {"name": activity_subtype}},
        "Distance (mi)": {"number": round(new_activity.get('distance', 0) / 1000 * 0.621371, 2)},
